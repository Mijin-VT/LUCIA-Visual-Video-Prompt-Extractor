...
2. **Unrestricted Expansion (Phase 2 - Optional):** Uses a text-only Video LLM to expand the visual data into a comprehensive, narrative, and unrestricted description.
...
### Step 1: Download Vision Images (Images folder)
1. Create a folder named `Images` in the project root directory.
2. Download a Vision-Language Model (VLM) in GGUF format and its corresponding CLIP projector (mmproj).
   - **Recommended Model:** Qwen3-VL-8B-Instruct-Q6_K
   - **Main Model File:** `Qwen3-VL-8B-Instruct-Q6_K.gguf`
   - **Projector File:** `mmproj-Qwen3-VL-8B-Instruct-F16.gguf`
3. You can find these files on Hugging Face (huggingface.co) by searching for "Qwen3-VL-8B-Instruct-Q6_K.gguf".
4. Place both downloaded files directly inside the `Images` folder.

### Step 2: Download Video AI Images (Video folder)
1. Create a folder named `Video` in the project root directory.
2. Download a Vision-Language Model (VLM) in GGUF format and its corresponding CLIP projector (mmproj).
...
https://huggingface.co/lmstudio-community/Qwen3-VL-8B-Instruct-GGUF/resolve/main/mmproj-Qwen3-VL-8B-Instruct-F16.gguf?download=true


2- Download and paste the Video folder
...
https://huggingface.co/lmstudio-community/Qwen2.5-VL-7B-Instruct-GGUF/resolve/main/mmproj-model-f16.gguf?download=true
...
3. **Sidebar Configuration:**
   ...
   - Select an **Video Model** from the dropdown if you downloaded one in Step 2.
4. **Main Interface:**
...
- **GGML_ASSERT failed / Memory Slot Error:** Occurs if too many std::string theme_name = settings.GetString("theme", "glassy_blue"); are sent at once. The application uses a "Temporal Collage" technique to mitigate this. Ensure you are using the updated `app.py`.
- **Model Rejection ("I cannot help with that"):** The vision model's safety filters are triggered by aggressive prompts. Use the default clinical/objective prompt provided in the UI, or rely on the Video Text Pipeline to expand the initial factual description.
...
- **Model Not Found:** Verify that the `.gguf` files are in the correct folders (`Images` or `Video`) and that the filenames match exactly what the application expects.

## Technical Notes

- **Context Size (-c 8192):** Optimized for 7B/8B Images on 16GB VRAM GPUs. Do not increase to 131072 unless using specialized hardware and Images.
